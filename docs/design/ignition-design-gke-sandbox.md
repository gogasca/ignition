...
- **GKE Dataplane V2** (`--enable-dataplane-v2`), set at creation, remains available for cluster-owned defense-in-depth policy. Public `network.internetAccess` is enforced by preconfigured GCP network profiles rather than client-authored Kubernetes `NetworkPolicy` rules.
