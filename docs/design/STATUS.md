...
| Warm-node capacity via balloon Pods | **PARTIAL** | Implemented; every overlay still ships `MIN_WARM`/`MIN_WARM_CPU=0` by default (a standing buffer costs real node money), so no deployment runs one out of the box. First real warm-vs-cold sample taken live on `anyscale-demo` (CPU): 5.07s warm vs 78.75s cold — see [shipped architecture §Startup SLO](ignition-shipped-architecture.md#startup-slo). Still not the 9s p95 this budget targets — that needs a real concurrent load run (`ignition_sandbox_stage_latency_seconds` is already emitted for when one happens), and the balloon Pod's image needed a same-registry mirror (`IGNITION_BALLOON_IMAGE`) to even schedule under this repo's own default-deny node egress — see [shipped architecture §7](ignition-shipped-architecture.md#7-warm-node-capacity). |
...
| WASI processes (`runtime: WASI`) | **SHIPPED** | `command[0]` is a `.wasm` module (WASI preview 1) that `sandbox-init` runs in an embedded engine (wazero, pure Go) *inside* the gVisor sandbox — no node, RuntimeClass, or image change. Working directory is the module's only filesystem; env is exactly `environment`; compiled modules are cached per sandbox; linear memory capped at 1 GiB per module. Any signal/cancel stops the module at once. No PTY. `ignitionctl exec --wasi`, SDK `runtime="WASI"`. A sandbox image embedding an older `sandbox-init` does not know the field and fails the process (exec format error) rather than misrunning it. |
