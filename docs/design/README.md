...
**Not built:** the exec byte path (`ignition-gateway` and the `sandbox-init` process supervisor are stubs and not deployed), `ignitionctl` (every subcommand returns `not implemented`), the public Image/Project/Secret/Event APIs (digest-pinned image admission is under implementation but is not a shipped public surface), and writable Volume / Session snapshot resources. `BARE_METAL` is represented in the contract but fails closed.
