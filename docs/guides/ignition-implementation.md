...
`POST .../images` resolves the request's `sourceRef` synchronously against its source registry (`internal/imagecatalog`, `permission image.create`). The network-reachability hole this used to leave open is closed (`internal/imagecatalog/guard.go`): `sourceRef` is rejected unless its registry host is on `IGNITION_IMAGE_REGISTRY_ALLOWLIST`, every resolved address is checked post-DNS against loopback/private/link-local/metadata ranges, and resolution is bounded by `IGNITION_IMAGE_RESOLVE_TIMEOUT_SECONDS` (default 30s) independent of the caller's own connection — see [Image delivery — Security status](../design/ignition-image-delivery.md#security-status) for what's still open (signature/provenance/scan, an Ignition-owned same-region copy). An empty allowlist logs a startup warning and still applies the address guard, but imposes no host restriction — set it in any deployment that grants `image.create` to an untrusted project member.
